...
function form_submit_up() {
	if (!$el("user").value || !$el("pass").value || ($el("email") && !$el("email").value)) {
		alert("You must fill in all of the fields.");
	}
	else if ($el("email") && (!$el("email").value.match(/^[A-Za-z0-9!#$%&'*+-\/=?^_`{|}~]+@([A-Za-z0-9-]+\.)+[A-Za-z]{2,6}$/))) {
		alert("Who're you trying to fool?  Proper email address.  Now.");
	}
